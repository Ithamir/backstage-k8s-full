...
if diff -u "$before_rbac" "$after_rbac" >/dev/null; then
  PASS=$((PASS + 1))
else
  FAIL=$((FAIL + 1))
  echo "FAIL: leaves RBAC policies untouched"
fi
if diff -u "$before_app_config" "$after_app_config" >/dev/null; then
  PASS=$((PASS + 1))
else
  FAIL=$((FAIL + 1))
  echo "FAIL: leaves app config untouched"
fi
if diff -u "$before_postgres" "$after_postgres" >/dev/null; then
  PASS=$((PASS + 1))
else
  FAIL=$((FAIL + 1))
  echo "FAIL: leaves postgres config untouched"
fi
