...
assert_contains "Git remote external data source exists" "$terraform_config" 'data "external" "git_remote"'
assert_contains "Git remote discovery script is wired" "$terraform_config" '${path.module}/scripts/git-remote.sh'
assert_not_contains "GitHub owner variable is deleted" "$terraform_config" 'variable "github_owner"'
assert_not_contains "GitHub repo variable is deleted" "$terraform_config" 'variable "github_repo"'
...
assert_contains "GitHub owner comes from git remote data" "$terraform_config" 'data.external.git_remote.result.owner'
assert_contains "GitHub repo comes from git remote data" "$terraform_config" 'data.external.git_remote.result.repo'
...
assert_contains "RBAC admin user is lowercased from discovered owner" "$terraform_config" 'rbac_admin_user  = lower(data.external.git_remote.result.owner)'
assert_contains "Platform identity ConfigMap is managed" "$terraform_config" 'resource "kubernetes_config_map_v1" "platform_identity"'
assert_contains "Platform identity includes discovered owner" "$terraform_config" 'GITHUB_OWNER = data.external.git_remote.result.owner'
assert_contains "Platform identity includes discovered repo" "$terraform_config" 'GITHUB_REPO  = data.external.git_remote.result.repo'
