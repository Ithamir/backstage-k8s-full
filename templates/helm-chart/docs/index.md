...
The `helm-chart` Template scaffolds a new workload chart under `charts/<name>/` and opens a pull request against this repository. Use it when a service needs the standard local-platform shape: a Deployment, Service, Gateway API `HTTPRoute`, catalog metadata, and a starter TechDocs site.
...
| `image` | Yes | Container image string placed in `values.yaml`. Defaults to `nginx:latest`. |
...
The template does not deploy the chart. It only creates a pull request containing the chart files. A reviewer should confirm the generated values are appropriate before merge and deployment.

## Pull Request Output

When the scaffolder runs, it fetches `templates/helm-chart/skeleton`, renders every `.njk` file with the submitted values, and writes the result to `charts/<name>/`.
