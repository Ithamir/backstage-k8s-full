...
```text
https://github.com/Itamar-Ratson/backstage-k8s-full/tree/main/charts/<component-name>/
```
...
That extracted path list is passed to `publish:github:pull-request` as `filesToDelete`. The workspace copy is deleted before publishing so the pull request is driven by explicit deletions rather than by leaving rendered files in the scaffolder workspace.
...
- The `filesToDelete` list is limited to `charts/<component-name>/`.
- The owning team has a plan to uninstall or otherwise retire the running release.
